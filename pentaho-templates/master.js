...
jQuery(document).ready(function($){
	...
	$(document).ajaxSuccess(function() {
		...
		$('[href=#'+dataClickedSection+']').addClass('active');
	...
	if ($.browser.msie && navigator.userAgent.indexOf('Trident')!==-1) {
		$('.play-overlay').append('<span class="play-overlay-button"></span>');
	};
	...
	//Show / Hide More + Less Link on Blog Archive block
	...
	//Float Blog Images (from Wordpress) right if they are small than 450px wide
  var blogimgwidth = $(".node-blog .field-name-field-body-blog img").first().attr("width");
  if(blogimgwidth < 451){
  	$(".node-blog .field-name-field-body-blog img").first().addClass("float-right");
  }
